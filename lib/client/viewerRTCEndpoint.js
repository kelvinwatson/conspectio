...
const viewerRTCEndpoint = (eventTag, viewerHandlers, videoEl) => {

    // viewer wants to initiate contact with broadcaster
    console.log('emit initiateView', eventTag);
    ...
    conspectio.socket.on('signal', (fromId, message, originId) => {
      console.log('got signal fromId:', fromId);
      if(message.type === 'offer') {
        ...
        if(conspectio.connections[compositeKey1]){
          ...
          console.log('VideoEL when creating new stream', videoEl);
          var newPC = new ConspectioViewer(fromId, viewerHandlers, originId, videoEl);
