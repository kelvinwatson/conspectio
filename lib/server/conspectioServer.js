  ...
  io.on('connection', (socket) => {
    ...
    socket.on('addBroadcaster', (eventId) => {
      ...
      if (CURR_DB === DB.FIREBASE){
        ...
        newBroadcasterFirebase['eventId_origin'] = newBroadcasterFirebase['eventId'] + '_' + newBroadcasterFirebase['origin'];
        newBroadcasterFirebase['socketId_origin'] = newBroadcasterFirebase['socketId'] + '_' + newBroadcasterFirebase['origin'];
        newBroadcasterFirebase['socketId_eventId_origin'] = newBroadcasterFirebase['socketId'] + '_' + newBroadcasterFirebase['eventId'] + '_' + newBroadcasterFirebase['origin'];
        ...
        let newBroadcaster = new ConspectioNode(socket.id, eventId); // add a new broadcaster asssociated with that event id into nodeTracker table
              ...
              nodeTrackerRef.orderByChild('eventId_origin').equalTo(currNode.eventId + '_' + '').once('value').then(function(snap){
                ...
                if (potentialOtherBroadcasters){
                  for(let k in potentialOtherBroadcasters){
                    if (potentialOtherBroadcasters.hasOwnProperty(k)){
                      let b = potentialOtherBroadcasters[k];
                      if (b.socketId !== newBroadcaster.socketId){
                        otherBroadcastersFirebase[k] = b;
                      }
                    }
                  }
