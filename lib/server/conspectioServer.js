      ...
      broadcastersList.forEach((broadcaster) => {
        ...
        while(!foundCapacity){
          ...
          if(broadcasterNode) {
            ...
            console.log('*****FIREBASE***** incrementing viewer count',firebase);
    ...
    socket.on('disconnect', () => {
      ...
      for(var i = 0; i < currNodeList.length; ++i) {
        ...
        if(currNode.origin) { // this is a VIEWER
          ...
          console.log('*****FIREBASE***** decrementing viewer count',firebase);
          firebase.database().ref('liveStreams/'+eventId).once('value').then(function(snap){
            var numViewers = snap && snap.val() || 0;
          ...
          //firebase remove the entry in the liveStreams table
          console.log('*****FIREBASE***** removing all viewers',firebase);
          firebase.database().ref('liveStreams/'+eventId).set(null);
        }
      }
    });
  });
